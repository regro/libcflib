...
def get_all_symbol_names(top_dir):
    ...
    for root, dirs, files in tqdm(os.walk(top_dir)):
        ...
        for file in _files:
            ...
            import_name = file_path_to_import(''.join(file_name.rpartition(module_import)[1:]))
            data = jedi.Script(path=file_name).complete()
            symbols_from_script = {k.full_name: k.type for k in data if k.full_name and module_import+'.' in k.full_name}
