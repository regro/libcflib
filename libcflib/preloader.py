    ...
    with ThreadPoolExecutor(max_workers=20) as pool:
        futures = [pool.submit(
            reap_package, path, package, dst, src_url, progress_callback=progress.update) for
                package, dst, src_url in sorted_files if (src_url not in known_bad_packages)
        ]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--known-bad-packages', help='name of a json file containing a list of urls to be skipped')
